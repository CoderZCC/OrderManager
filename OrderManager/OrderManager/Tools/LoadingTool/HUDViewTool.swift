...
@objc extension UIResponder {
    ...
    func showProgress(progress: CGFloat, text: String) {
        ...
        if Thread.isMainThread {
            ...
            //SwiftLoadingTool.showDownloadProgress(progress: progress)
    ...
    func showLoadingSupportClick() {
        ...
        if Thread.isMainThread {
            ...
            //self.hideHUD()
            //SwiftNotice.wait(isSupportClick: true)
        ...
        if Thread.isMainThread {
            ...
            if !HUDViewTool.windows.isEmpty {
                ...
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) {
                    ...
                    //SwiftNotice.showText(text, autoClear: true, autoClearTime: 1)
                }
...
class HUDViewTool: UIView {
    ...
    /// 菊花+文字
    @IBOutlet weak var circleTextView: UIView!
    @IBOutlet weak var aiView1: UIActivityIndicatorView!
    @IBOutlet weak var titleL1: UILabel!
    
    /// 菊花
    @IBOutlet weak var circleView: UIView!
    @IBOutlet weak var aiView2: UIActivityIndicatorView!
    
    /// 文字
    @IBOutlet weak var textView: UIView!
    @IBOutlet weak var titleL3: UILabel!
    @IBOutlet weak var textWidthCons: NSLayoutConstraint!
    @IBOutlet weak var textHeightCons: NSLayoutConstraint!
  
    /// 进度+文字
    @IBOutlet weak var progressView: UIView!
    @IBOutlet weak var huanView: UIView!
    @IBOutlet weak var titleL4: UILabel!
    
    /// 展示的Windows
    static var windows: Array<UIWindow?> = Array<UIWindow?>()
    
    /// 是否支持点击取消 默认为不支持
    static var isClickCancle: Bool = false
    
    static let textMaxW: CGFloat = kWidth - 100.0
    static let textMinW: CGFloat = 100.0
    static let textMinH: CGFloat = 30.0
